            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ flex: '0 0 68%', minWidth: 0 }}>
                ...
                {b.gallerySelections?.map(g => <div key={g.galleryId} style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,flexWrap:'wrap',marginTop:8,padding:'8px 10px',background:'#e3eee9',borderRadius:6,fontSize:13}}>
                  ...
                  {g.clientUrl ? <div style={{display:'flex',gap:8,flexWrap:'wrap'}}><a href={`/g/${g.galleryId.slice(0,12)}`} target="_blank" rel="noopener noreferrer" style={{color:'#415e58',fontWeight:700,whiteSpace:'nowrap'}}>Open Client Gallery</a><a href={photoSelectGalleryActionUrl(b,g.galleryId,'update-basic')} target="_blank" rel="noopener noreferrer" style={{border:'1px solid #557970',background:'#fff',padding:'5px 9px',borderRadius:6,color:'#354c47',fontWeight:700,textDecoration:'none',whiteSpace:'nowrap'}}>Update Basic Retouch</a>{g.submitted&&<a href={photoSelectGalleryActionUrl(b,g.galleryId,'unlock-selection')} target="_blank" rel="noopener noreferrer" style={{border:'1px solid #b87962',background:'#fff',padding:'5px 9px',borderRadius:6,color:'#704333',fontWeight:700,textDecoration:'none',whiteSpace:'nowrap'}}>Allow Client to Change Selection</a>}{g.locked&&<a href={photoSelectGalleryActionUrl(b,g.galleryId,'upload-further')} target="_blank" rel="noopener noreferrer" style={{border:'1px solid #8d6fa8',background:'#8d6fa8',padding:'5px 9px',borderRadius:6,color:'#fff',fontWeight:700,textDecoration:'none',whiteSpace:'nowrap'}}>Upload Further Retouch</a>}{g.deliveredAt&&<button type="button" onClick={()=>copyClientGalleryLink(b.id,g.galleryId)} style={{border:'1px solid #557970',background:'#557970',padding:'5px 9px',borderRadius:6,color:'#fff',fontWeight:700,cursor:'pointer'}}>Copy Further Retouch WhatsApp Message</button>}<button type="button" onClick={()=>setManageGallery({bookingId:b.id,galleryId:g.galleryId})} style={{border:'1px solid #557970',background:'#fff',padding:'5px 9px',borderRadius:6,color:'#354c47',fontWeight:700,cursor:'pointer'}}>Manage Gallery</button></div> : <span style={{color:'#8b5b43'}}>Link this Gallery again in PhotoSelect Pro to enable the mobile client link.</span>}
                </div>)}
