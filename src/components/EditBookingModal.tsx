...
export default function EditBookingModal({
  ...
  const [inspirationExisting,setInspirationExisting]=useState<string[]>(booking.inspirationReferencePhotoUrls||[]);
  const [inspirationPending,setInspirationPending]=useState<{file:File;previewUrl:string}[]>([]);
  ...
  async function save() {
    ...
    try {
      ...
      const setupResponse=await fetch(`/api/admin/bookings/${booking.id}/setup-choice`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({setupSelectionCount:setupCount,setupSelections,inspirationReferencePhotoUrls,setupChoiceNotes:booking.setupChoiceNotes||''})});
